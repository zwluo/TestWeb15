...
(function($) {
    ...
    $.fn.bootstrapTreeTable = function(options, param) {
        ...
        var initServer = function(parms) {
            ...
            // 添加加载loading
            var $loading = '<tr><td rowspan="' + options.columns.length + '"><div style="display: block;text-align: center;">正在努力地加载数据中，请稍候……</div></td></tr>'
            $tbody.html($loading);
            if (options.url) {
                $.ajax({
                    type: options.type,
                    ...
                    error: function(xhr, textStatus) {
                        var _errorMsg = '<tr><td rowspan="' + options.columns.length + '"><div style="display: block;text-align: center;">' + xhr.responseText + '</div></td></tr>'
        ...
        var renderTable = function(data) {
            ...
            if (!data || data.length <= 0) {
                var _empty = '<tr><td rowspan="' + options.columns.length + '"><div style="display: block;text-align: center;">没有找到匹配的记录</div></td></tr>'
                $tbody.html(_empty);
                return;
            }
            // 缓存并格式化数据
